...
import { sanitizeBlobError } from '../product-detection/errors.ts';
import { getLocalContentBlobToken } from './config.ts';
...
export function contentBlobAuthOptions(
  auth?: ContentBlobAuth,
  environment: NodeJS.ProcessEnv = process.env
): ContentBlobAuth {
  if (auth) return auth;
  const localToken = environment.BLOB_READ_WRITE_TOKEN?.trim();
  if (environment.NODE_ENV !== 'production' && localToken) return { token: localToken };
  // En producción, el SDK resuelve el OIDC del runtime y BLOB_STORE_ID.
  return {};
}

async function resolveContentBlobAuth(auth?: ContentBlobAuth): Promise<ContentBlobAuth> {
  if (auth) return auth;
  const localToken = await getLocalContentBlobToken();
  return localToken ? { token: localToken } : {};
}

function blobDiagnostic(error: unknown): string {
  return JSON.stringify(sanitizeBlobError(error));
}

function publicBlobError(error: unknown, operation: 'read' | 'write' = 'write'): ContentError {
  ...
  return operation === 'read'
    ? new ContentError('No se pudo leer esta imagen de la biblioteca. Inténtalo de nuevo.', 'BLOB_LECTURA', 503)
    : new ContentError('No se pudo guardar esta imagen en la biblioteca. Las demás imágenes válidas se conservarán y podrás reintentar esta vista.', 'BLOB_GUARDADO', 503);
}
...
export async function uploadGeneratedContentImage(
  ...
  try {
    const auth = await resolveContentBlobAuth(options.auth);
...
export async function uploadContentReference(
  ...
  try {
    const stored = await (options.uploader ?? put)(pathname, Buffer.from(bytes), {
      ...await resolveContentBlobAuth(options.auth),
      access: 'private',
      addRandomSuffix: false,
      allowOverwrite: false,
...
export async function deleteGeneratedContentImages(pathnames: string[]): Promise<void> {
  ...
  try {
    await del(pathnames, await resolveContentBlobAuth());
  } catch {
...
export async function streamPrivateContentImage(
  ...
  try {
    const result = await (options.getter ?? get)(urlOrPathname, { ...await resolveContentBlobAuth(options.auth), access: 'private' });
    ...
    throw publicBlobError(error, 'read');
  }
}
...
export async function readContentImageDataUrl(urlOrPathname: string): Promise<string> {
  ...
  try {
    const result = await get(urlOrPathname, { ...await resolveContentBlobAuth(), access: 'private' });
